...
public static class RuntimeEmitter
{
    ...
    public static void Execute(BoundBlockStatement program)
    {
        if (program is null)
        {
        ...
        var assemblyName = new AssemblyName($"BottleSharp.Dynamic.{Guid.NewGuid():N}");
        ...
        var entry = runtimeType.GetMethod("Main", BindingFlags.Public | BindingFlags.Static)
            ?? throw new InvalidOperationException("Generated entry point was not found.");
